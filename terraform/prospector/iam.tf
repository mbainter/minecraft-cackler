resource "aws_iam_user" "claimjumper" {
  name = "claimjumper"
}

data "aws_iam_policy_document" "claimjumper" {
  statement {
...
resource "aws_iam_policy" "claimjumper" {
  name        = "ClaimJumperEC2Access"
  path        = "/"
  description = "Access policy for the ClaimJumper tool to start and query the Minecraft server"

  policy = data.aws_iam_policy_document.claimjumper.json
}

resource "aws_iam_access_key" "claimjumper" {
  user    = aws_iam_user.claimjumper.name
  pgp_key = "keybase:mbainter"
}

resource "aws_iam_user_policy_attachment" "claimjumper" {
  user       = aws_iam_user.claimjumper.name
  policy_arn = aws_iam_policy.claimjumper.arn
}
